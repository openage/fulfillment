...
const getLocality = (location) => {
    ...
    return client.getPromise(locationConfig.localityUrl, args)
        .then((response) => {
            location.name = response.data.results.length !== 0 ? response.data.results[2] ? response.data.results[2].formatted_address : response.data.results[0].formatted_address : ''
